...
exports.replaceGlobals = (skowix, string) => {
    return string
        .replace(/{servername}/g, skowix.config.FiveMServerName)
        .replace(/{invite}/g, skowix.config.DiscordInviteLink)
...
const log = {
    ...
    write: (content, { color = "\x1b[37m", tag = "LOG", error = false } = {}) => {
        ...
        stream.write(`\x1b[1;36m[skx_discordbot]\x1b[0m[${log.timestamp()}]${color}[${tag}]: ${log.clean(content)}\x1b[0m\n`);
        return false;
    },
    ...
    /** Special case log handler for skowixdiscord specifc logs
     *  Or just outright void spammy logging (heartbeats)
     * @param {string|object} err - error to process */
    handler: (type, err) => {
        const e = err.toString();
        if (e.includes("[DISALLOWED_INTENTS]")) log.error("NIE WŁĄCZYŁEŚ INTENCJI w discord dev portal.");
        else if (e.includes("[TOKEN_INVALID]")) log.error("TWÓJ TOKEN API DISCORD JEST NIEWAŻNY - WYGENERUJ NOWY I ZAKTUALIZUJ CONFIG.");
        else if (e.includes("Missing Access")) log.error("BRAK UPRAWNIEŃ DO TWORZENIA KOMEND - Musisz ponownie zaprosić bota na swój serwer z pelnymi permisjami.");
    ...
    /** Special case error handler for skowixdiscord specifc errors
